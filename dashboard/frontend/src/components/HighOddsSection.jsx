...
import { ChevronDown, Sparkles, TrendingUp, Target } from 'lucide-react'
...
function MatchRow({ match, index, onAnalyze }) {
  ...
  return (
    <motion.div
      ...
      className={`border-b border-slate-800/45 px-4 py-3 transition-colors hover:bg-amber-500/10 ${
        isBetRecommended ? 'bg-amber-500/15' : 'bg-amber-500/5'
      }`}
    >
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)_minmax(0,0.9fr)_auto] items-center">
        <div className="space-y-1.5 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400">
            ...
            <span className="text-amber-300 font-semibold">Predicted {predictedOdds.toFixed(2)}</span>
          </div>
        ...
        <div className="text-center text-xs text-slate-400">
          ...
          <span className={`ml-1 uiverse-pill text-xs ${
